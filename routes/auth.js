  ...
  userService.getOne(email).then((data) => {
    ...
    crypto.pbkdf2(password, data.salt, 310000, 32, 'sha256', function(err, hashedPassword) {
      ...
      try {
        ...
        return res.status(200).json({message: 'Successfully logged in', id: data.id, email: data.email, token: token});
      } catch (err) {
        return res.status(500).json({message: 'Something went wrong with creating JWT token'});
      }
    });
  });
});
...
router.post("/signup", async (req, res, next) => {
  ...
  if (user != null) {
    return res.status(400).json({ message: 'Provided email is already in use.' });
  }
  const salt = crypto.randomBytes(16);
  crypto.pbkdf2(password, salt, 310000, 32, 'sha256', function(err, encryptedPassword) {
    if (err) { return next(err); }
    userService.create(name, email, encryptedPassword, salt)
